...
public interface PlaylistRepository extends JpaRepository<PlaylistEntity, UUID> {
    ...
    List<PlaylistEntity> findAllByChanelIdAndStatus(Integer id, PlaylistStatus status, Sort sort);

    Optional<PlaylistEntity> findById(Integer id);

    @Query("from PlaylistEntity where chanel.profile.id = :profileId")
    ...
    @Query("SELECT pl.id as pl_id,pl.name as pl_name,pl.description as pl_description,pl.status as pl_status,pl.order_num as pl_order_num, " +
            "ch.id as ch_id, ch.name as ch_name, ch.photo as ch_photo_id, " +
            "pr.id as pr_id, pr.name as pr_name,pr.surname as pr_surname,pr.photo as pr_photo_id " +
